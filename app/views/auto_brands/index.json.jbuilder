json.array! @auto_brands do |ab|
    ...
    json.set! 'ams' do
        json.array! ab.auto_models.where(service_level: 1).asc(:name_pinyin) do |am|
            json.name am.name
            json._id am.id
            json.set! 'asms' do
                params[:city_id] = '5307033e098e719c45000043' if params[:city_id].blank?
                json.array! am.auto_submodels.where(data_source: 2, service_level: 1, :city_ids.in => [params[:city_id]]).asc(:name) do |asm|
