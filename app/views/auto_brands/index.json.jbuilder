json.array! @auto_brands do |ab|
    ...
    json.set! 'ams' do
        json.array! ab.auto_models.where(service_level: 1).asc(:name_pinyin).select { |am| am.auto_submodels.where(data_source: 2, service_level: 1).exists? } do |am|
            json.name am.name
            json.set! 'asms' do
                params[:city] = I18n.t(:beijing) if params[:city].blank?
                json.array! am.auto_submodels.where(data_source: 2, service_level: 1).asc(:name).select {|asm| asm.cities.where(name: params[:city]).exists? } do |asm|
